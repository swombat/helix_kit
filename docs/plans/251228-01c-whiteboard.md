...
**Previous:** 251228-01b (approved by DHH review)
...
This is the final, polished version ready for implementation.
...
class WhiteboardTool < RubyLLM::Tool
  ...
  param :content, type: :string,
        desc: "Board content in markdown. For update: pass empty string to clear, omit to keep unchanged.",
        required: false
  ...
  # Note on content parameter: uses `content.nil?` (not `.blank?`) deliberately.
  # This allows passing an empty string to clear content while omitting the
  # parameter preserves existing content. Deletion is a soft-delete that
  # preserves content for potential restore.
...
class WhiteboardToolTest < ActiveSupport::TestCase
  ...
  test "get includes editor_name from polymorphic association" do
    board = whiteboards(:project_notes)
    result = @tool.execute(action: "get", board_id: board.obfuscated_id)
...
The `update_board` action uses `content.nil?` instead of `content.blank?` deliberately:
