# GitHub Repository Integration (v3 - Final)
...
This third iteration addresses final DHH feedback: adds format validation on `repository_full_name`, aligns guard clause style with Oura's multi-line pattern, extracts `ready_to_sync?` predicate, and makes `disconnect!` consistent with Oura's approach.
...
Note: `disconnect!` clears tokens and data but preserves `github_username`, matching Oura's pattern of retaining user-identifying info after disconnect.
...
Uses `current_account` from `AccountScoping` concern (already included in `ApplicationController`). Guard clause style matches Oura's multi-line `if`/`unless` blocks with explicit `return`.
...
- [ ] **Model tests**: `GithubIntegration` validations (including `repository_full_name` format), `commits_context` formatting, `disconnect!`, `ready_to_sync?`, `sync_all_due!`
...
- **Token revocation**: GitHub API returns 401 -> clear access_token and github_username, raise error
...
- **Invalid repo name injection**: Format validation (`/\A[\w\-\.]+\/[\w\-\.]+\z/`) prevents path traversal or injection via `repository_full_name` which is interpolated into API URLs

## Changes from v2

1. **Format validation on `repository_full_name`**: Added `validates :repository_full_name, format: { with: /\A[\w\-\.]+\/[\w\-\.]+\z/ }, allow_nil: true` to prevent injection via the parameter that gets interpolated into API URLs
2. **Guard clause style matches Oura**: Replaced one-liner `redirect_to ... and return` with multi-line `unless`/`if` blocks with explicit `return`, matching the Oura controller exactly
3. **Extracted `ready_to_sync?` predicate**: Simplifies the `sync` controller action and `sync_commits!` model method with a single readable predicate
4. **`disconnect!` preserves `github_username`**: Matches Oura's pattern of retaining user-identifying info (Oura does not clear its equivalent on disconnect). Only clears tokens, repo, and cached data
