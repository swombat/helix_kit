...
Instead, Pay uses webhooks to create the the subscription in the database. The Paddle `customer` field is required during checkout to associate the subscription with the correct `Pay::Customer`.
...
Then using either the Javascript `Paddle.Checkout.open` method or the Paddle Button Checkout, pass the `customer` object and an array of items to subscribe to:
...
Lemon Squeezy offers 2 checkout flows: a hosted checkout and a checkout overlay. When creating a Product in the Lemon Squeezy dashboard, clicking the "Share" button will provide you with the URLs for either checkout flow.
...
It's currently not possible to pass a pre-existing Customer ID to Lemon Squeezy, so you can use the passthrough method to associate the subscription with the correct `Pay::Customer`.

You can pass additional options to the checkout session. You can view the [supported fields here](https://docs.lemonsqueezy.com/help/checkout/prefilling-checkout-fields) and the [custom data field here](https://docs.lemonsqueezy.com/help/checkout/passing-custom-data).
...
Individual subscriptions provide several helper methods to check their state:
...
For Paddle specifically, there's an additional option to get a subscription cancel URL:
...
Important notes about immediate cancellation:
- The subscription will be canceled immediately
- You *cannot* resume the subscription
- If you want to refund the customer for remaining time, you'll need to calculate and issue a refund separately
...
A user can resume their canceled subscription during the grace period:
