...
```
User clicks deep link from the app -> opens Telegram -> sends /start
...
- [ ] **User-facing deep link in chat UI**

The deep link is user-specific (signed with `message_verifier` for the current user). It must be shown to each end user, not just admins. Two places to surface it:

**a) Chat sidebar / conversation header** -- when the agent has Telegram configured, show a small prompt to the user if they do not yet have a `TelegramSubscription` for this agent:

Pass `telegram_deep_link` as a shared prop from the chat show controller when the agent has Telegram configured and the current user has no active subscription:

```ruby
# In the controller rendering the chat view
telegram_sub = @chat.agent&.telegram_subscriptions&.find_by(user: current_user, blocked: false)
telegram_deep_link = if @chat.agent&.telegram_configured? && telegram_sub.nil?
  @chat.agent.telegram_deep_link_for(current_user)
end
```

In the Svelte component, render a dismissible banner:

```svelte
{#if telegramDeepLink}
  <div class="p-3 rounded-lg bg-muted/50 text-sm space-y-1">
    <p class="font-medium">Get notified on Telegram</p>
    <p class="text-muted-foreground text-xs">
      Receive a notification when {agent.name} reaches out.
    </p>
    <a href={telegramDeepLink} target="_blank" class="text-xs underline">
      Connect on Telegram
    </a>
  </div>
{/if}
...
**b) Agent edit page (admin)** -- show the Telegram configuration fields. The deep link shown here is for the admin's own subscription (useful for testing). Make this clear in the UI:
...
- `telegram_webhook_token` stored as column for O(1) lookup -- derived from agent ID + bot token, not guessable without both
- `telegram_webhook_secret` derived from `Rails.application.key_generator` -- impossible to forge even with the bot token, since it requires the app's `secret_key_base`
- Deep link tokens use `Rails.application.message_verifier` with 7-day expiry -- signed, tamper-proof, time-limited
- Deep link verification scopes user lookup to `agent.account.users` -- a valid token from one account cannot register against an agent in a different account
...
- [ ] **Concern tests** (`test/models/concerns/telegram_notifiable_test.rb`)
  ...
  - `set_telegram_webhook!` logs error on failure
  - `delete_telegram_webhook!` logs error on failure
...
- [ ] **TelegramWebhooksController tests**
  ...
  - 200 and enqueues job for valid requests (agent passed as GlobalID)
...
- [ ] **ProcessTelegramUpdateJob tests**
  ...
  - Cross-account token rejected (user in account A, agent in account B)
...
- **Bot token invalid**: `manage_telegram_webhook` callback fires `set_telegram_webhook!` which fails -- logged via `Rails.logger.error`, not blocking
- **Webhook setup/teardown failures**: Always logged with agent ID for debugging, never swallowed silently
- **Deep link expired**: `message_verifier.verified` returns `nil`, user gets "couldn't identify" message
- **Cross-account deep link**: `agent.account.users.find_by(id: user_id)` returns `nil` -- a signed token from account A cannot register against an agent in account B
...
- Per-user notification preferences (e.g. settings page with all agents)
