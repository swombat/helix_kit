# Audio File Storage, Playback, and LLM Access (v2)
...
3. **FetchAudioTool** -- A tool registered only for audio-capable models (currently Gemini) that lets the agent retrieve the original audio file into its context by obfuscated message ID.
...
- [ ] Extract shared `resolve_attachment_path` private method
...
Note: `rails_blob_path` is used instead of `rails_blob_url` with `only_path: true`. Cleaner, more honest, same result.
...
- [ ] Add comment explaining safety of `pendingAudioSignedId` via `submitting` guard
...
### 13. `create_with_message!` -- No Changes Needed
...
- [ ] `test "resolve_attachment_path returns nil for unattached"`
- [ ] `test "file_paths_for_llm uses resolve_attachment_path"`
...
| File | Changes |
|------|---------|
| `app/models/message.rb` | Add `has_one_attached :audio_recording`, `audio_url`, extract `resolve_attachment_path`, refactor `file_paths_for_llm`, add `audio_path_for_llm`, `audio_source` + `audio_url` to json_attributes, `audio/webm` to acceptable types |
...
| `test/models/message_test.rb` | Add resolve_attachment_path and audio tests |
