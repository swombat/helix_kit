...
class OpenRouterGPT5Test < ActiveSupport::TestCase

  test "gets response from openai/gpt-5 model" do
    VCR.use_cassette("openrouter_gpt5_basic_response") do
      ...
      assert_equal "openai/gpt-5", response["model"]
  ...
  test "streams text response from openai/gpt-5 model" do
    VCR.use_cassette("openrouter_gpt5_stream_text_response") do
  ...
  test "gets json response from openai/gpt-5 model" do
    VCR.use_cassette("openrouter_gpt5_json_response") do
      response = OpenRouterApi.new.get_response(
        params: {
          model: "openai/gpt-5",
      ...
      assert response["model"].include?("gpt-5"), "Expected model to include 'gpt-5'"
  ...
  test "streams json response from openai/gpt-5 model" do
    VCR.use_cassette("openrouter_gpt5_stream_json_response") do
      # Variables to track streaming
