...
**Revision:** B (incorporating DHH feedback)
...
| Issue | Resolution |
|-------|------------|
| Duplicated validation in tools | Removed; model validates via `ActiveRecord::RecordInvalid` rescue |
| Two nearly-identical tools | Merged into single `SaveMemoryTool` with `memory_type` parameter |
| `memory_context` too long | Extracted `core_memory_section` and `journal_memory_section` private helpers |
| `memories_count` fires two queries | Now uses single `memories.group(:memory_type).count` query |
...
Add method for memory injection with extracted private helpers:
...
- [ ] Add `memories_count` (with grouped count) to Agent model
...
- [ ] Agent#core_memory_section tests
- [ ] Agent#journal_memory_section tests
