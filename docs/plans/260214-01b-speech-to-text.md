...
**Date**: 2026-02-14
**Status**: Draft (second iteration, incorporating DHH feedback)
...
Key changes from first iteration:
...
- **No `ENV` fallback**: Credentials only, matching the convention used by OuraApi and GithubApi. If someone needs an ENV-based key for development, they use the development credentials file.
...
Key changes from first iteration:
- **`mediaRecorder` and `audioChunks` are plain `let` variables, not `$state`**: These are internal callback state, not reactive UI state. Nothing in the template reads them directly. Only `state` drives the UI, so only `state` needs `$state`. The removed `isBusy` derived was also unused in the template.
- **`MIN_AUDIO_BYTES` named constant**: The magic number 1000 now has a name at the top of the script block.

CSRF pattern note: This component uses the same `document.querySelector('meta[name="csrf-token"]')?.content` pattern found throughout the existing codebase. The `show.svelte` page alone has six instances of this exact pattern (lines 337, 736, 923, 959, 985, 1008). There is no shared CSRF utility in the codebase -- this is the first non-Inertia fetch call in a standalone component (all others are inline in page-level `show.svelte`). Extracting a shared `csrfFetch` utility would be a worthwhile cleanup, but it should cover the existing six instances too. That cleanup is out of scope for this feature.
...
Note: The first iteration used `as: :json` on the POST requests, which can conflict with `fixture_file_upload` (multipart form data and JSON content type are mutually exclusive). Dropped `as: :json` and let the content type be inferred from the multipart form data. The controller responds with JSON regardless because it uses `render json:`.
...
class ElevenLabsSttTest < ActiveSupport::TestCase
  ...
  test "raises when API key is not configured" do
    Rails.application.credentials.stubs(:dig).with(:ai, :elevenlabs, :api_key).returns(nil)

    error = assert_raises(ElevenLabsStt::Error) { ElevenLabsStt.transcribe(@audio) }
    assert_includes error.message, "not configured"
  end

  test "sends correct headers and form fields" do
...
The tests cover all five branches in `handle_response` (200, 401, 429, 422, else) plus edge cases (empty text, whitespace text, unparseable 422 body, missing API key). Uses WebMock's `stub_request` to mock HTTP calls, matching the established pattern from `WebToolTest`.
...
2. **Extract shared CSRF fetch utility**: The `document.querySelector('meta[name="csrf-token"]')?.content` pattern appears 6+ times in `show.svelte` and in other pages. A shared `csrfFetch` wrapper would DRY this up.
