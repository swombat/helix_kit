...
- [ ] Add `Chat#trigger_mentioned_agents!(content)` method
...
- **Loads agents via association**: Only matches agents actually in this chat.
...
- [ ] Replace the existing AI trigger line with an explicit `if/else`
...
The `if/else` reads like prose: "If it is a group chat, trigger mentioned agents; otherwise, trigger the AI response." No hidden guard clauses, no implicit mutual exclusion. The intent is on the surface.

### Step 3: Write model tests

- [ ] Add 7 focused tests to `test/models/chat_test.rb`

Each test covers a distinct behavior. No tests that merely exercise Ruby's regex engine.
...
- [ ] Add 3 integration tests to `test/controllers/messages_controller_test.rb`
...
**Agent names with special characters**: `Regexp.escape` handles names like "C++Bot".
...
**Name that is a substring of another**: `\bChris\b` does not match inside "Christine" because there is no word boundary between "Chris" and "tine". The message "Chris and Christine" correctly matches both.
...
## Changes from First Iteration (03a)

| First iteration | This iteration |
|---|---|
| Two public methods + one private helper | Single `trigger_mentioned_agents!` method, inline regex |
| `auto_trigger_mentioned_agents!` naming | `trigger_mentioned_agents!` -- parallels existing methods |
| Silent `respondable?` guard in model | Dropped; controller already enforces via `before_action` |
| Step 4: modify `create_with_message!` | Dropped entirely -- speculative, unlikely scenario |
| Adjacent lines with implicit mutual exclusion | Explicit `if/else` to communicate mutual exclusivity |
| `create_group_chat` test helper | Inline setup in each test |
| 11 mention detection tests + 4 trigger tests | 7 focused model tests + 3 controller tests |
