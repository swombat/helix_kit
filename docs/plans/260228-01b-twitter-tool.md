...
class TwitterToolTest < ActiveSupport::TestCase
  ...
  test "post creates tweet log record" do
    ...
    assert_difference "TweetLog.count", 1 do
      build_tool.execute(text: "Logged tweet")
    end
  ...
  test "handles rate limit errors" do
    ...
    result = build_tool.execute(text: "Rate limited tweet")
  ...
  test "handles auth errors" do
    ...
    result = build_tool.execute(text: "Unauthorized tweet")
  ...
  test "allows tweets at exactly 280 characters" do
    ...
    result = build_tool.execute(text: text)
...
Testing notes:
- Uses `webmock` for HTTP stubbing, consistent with `WebToolTest`.
- No mocks, stubs, or fake objects per project rules.
- Covers: missing integration, disabled integration, disconnected integration, text too long, successful post, audit log creation, rate limits, auth errors, boundary (exactly 280 chars).
- No "invalid action" test needed since there is no action parameter.
- The exact URL stubbed (`https://api.twitter.com/2/tweets`) matches the `x` gem's default base URL (`https://api.twitter.com/2/`) plus the `tweets` path.
...
| Change | Rationale |
|--------|-----------|
| Dropped `action` parameter and polymorphic dispatch | Tool does one thing; YAGNI |
| Fixed JSON construction to `{ text: text }.to_json` | Idiomatic Ruby, not manual string building |
| Dropped `XApi::RateLimited` exception subclass | Single `Error` class, rate limit info in message string |
| Dropped `posted_at` column from `tweet_logs` | Redundant with `created_at` |
| Fixed `TweetLog.agent` to be consistently non-optional | `null: false` in migration, non-optional `belongs_to` in model |
| Simplified to single `error(msg)` helper | One-liner, inline everything else |
| Fixed controller to use `build_x_integration` + `update!` | Single database write instead of create-then-update |
| Pinned `x` gem response as Hash | Gem returns parsed Hash by default, no conditional needed |
