...
A simplified JSON API for AI clients (Claude Code, etc.) to interact with conversations and whiteboards. This final revision applies DHH's second review fixes:
...
| ApiKey model | ~35 | `app/models/api_key.rb` |
...
## 14. Changes from v2 (260115-01b)

### 1. SHA256 Token Hashing (Previously BCrypt)
...
Why: BCrypt is intentionally slow for password hashing. API tokens need fast verification. SHA256 gives us a single indexed query instead of iterating through candidates.

### 2. Fixed Transcript Variable Passing

**Before:**
```ruby
def generate_summary_from_llm
  transcript = messages.where(...)  # Built but never used
  prompt = Prompt.new(...)
  response = prompt.execute_to_string  # transcript not passed!
```

**After:**
```ruby
def generate_summary_from_llm
  transcript_lines = messages.where(...)
  prompt = Prompt.new(...)
  response = prompt.render(messages: transcript_lines)
  prompt.execute_to_string&.squish&.truncate_words(SUMMARY_MAX_WORDS)
```

Why: The Prompt class expects args to be passed either to `render()` or set on `@args`. We now pass the transcript array directly to `render()`.

### 3. Consistent Prompt Template
...
Why: Matches the existing `generate_title` template pattern. Receives an array, iterates over it.

### 4. Removed Prefix Index

**Before:** Index on `token_prefix` for candidate filtering
**After:** Only index on `token_digest`

Why: With SHA256, we do a direct lookup by digest. The prefix is kept for display purposes only.
