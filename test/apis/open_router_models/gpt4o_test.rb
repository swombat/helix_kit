...
class OpenRouterGPT4OTest < ActiveSupport::TestCase

  test "gets response from openai/chatgpt-4o-latest model" do
    VCR.use_cassette("openrouter_gpt4o_basic_response") do
      ...
      assert_equal "openai/chatgpt-4o-latest", response["model"]
      ...
      assert_equal "Of course! I'm here to help. What do you need assistance with?", text_content

      # No longer checking the content in the standardized format as we're using the OpenAI format directly
    end
  end

  test "streams text response from openai/chatgpt-4o-latest model" do
    VCR.use_cassette("openrouter_gpt4o_stream_text_response") do
      ...
      # Verify specific content of the poem (using the actual content from the VCR cassette)
      expected_poem = [ "In lines of logic, thoughts align,",
                       "A world is built with every sign.",
                       "From loops that turn to ifs that guide," ]

      # assert_equal text_content, expected_response

      expected_poem.each do |line|
        assert_includes text_content, line
      end
  ...
  test "gets json response from openai/chatgpt-4o-latest model" do
    VCR.use_cassette("openrouter_gpt4o_json_response") do
      response = OpenRouterApi.new.get_response(
        params: {
          model: "openai/chatgpt-4o-latest",
      ...
      assert response["model"].include?("chatgpt-4o"), "Expected model to include 'chatgpt-4o'"
  ...
  test "streams json response from openai/chatgpt-4o-latest model" do
    VCR.use_cassette("openrouter_gpt4o_stream_json_response") do
      # Variables to track streaming
