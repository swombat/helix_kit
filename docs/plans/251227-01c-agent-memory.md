...
**Revision:** C (final, incorporating all DHH feedback)
...
Memories are private per-agent, auto-injected after system prompt, and created via a single unified tool. Admins can review memories on the agent edit page.
...
|----------|---------|
| A | Initial spec |
| B | Merged two tools into one, removed duplicated validation, extracted private helpers, used grouped count |
| C | Fixed N+1 query in helpers (load once, pass down), fixed `memories_count` hash key handling |
...
Add method for memory injection with extracted private helpers. Note: memories are loaded once and passed to helpers to avoid N+1 queries:
...
### SaveMemoryTool (Unified)

**File:** `app/tools/save_memory_tool.rb`
...
This unified tool:
- Eliminates duplication between two nearly-identical tools
- Lets the model handle validation via `ActiveRecord::RecordInvalid` rescue
- Uses `AgentMemory.memory_types.key?` for maintainable type validation
- Provides appropriate response based on memory type
- One tool, one concept - the LLM can handle choosing the type
  ...
  <CardContent>
    {#if memories.length === 0}
      <p class="text-sm text-muted-foreground py-4">
        This agent has no memories yet. Memories will appear here as the agent creates them using the save_memory tool.
      </p>
    {:else}
...
- [ ] Add `core_memory_section` private helper to Agent model
- [ ] Add `journal_memory_section` private helper to Agent model
- [ ] Add `memories_count` (with grouped count and `.fetch`) to Agent model
- [ ] Update Agent `json_attributes` to include `memories_count`
- [ ] Update `Chat#system_message_for` to inject memories

### Tools
- [ ] Create `app/tools/save_memory_tool.rb`
...
- [ ] Agent#memory_context tests (verify single query)
- [ ] Agent#memories_count tests (verify hash keys)
- [ ] SaveMemoryTool tests (both journal and core types)
...
test "memory_context executes single query" do
  agent = @account.agents.create!(name: "With Memories")
  ...
  assert_equal 1, query_count, "memory_context should execute exactly one query"
end

test "memories_count uses single grouped query" do
  agent = @account.agents.create!(name: "With Memories")
  agent.memories.create!(content: "Core 1", memory_type: :core)
...
**File:** `test/tools/save_memory_tool_test.rb`
...
| Agent model additions | 30 | `app/models/agent.rb` |
| Chat model update | 15 | `app/models/chat.rb` |
| SaveMemoryTool | 45 | `app/tools/save_memory_tool.rb` |
...
- **No memory leakage between agents**: The system message (containing memories) is built fresh per-agent via `build_context_for_agent(agent)` and passed to an ephemeral RubyLLM chat instance. It is **never persisted** to the messages table. Each agent only sees their own memories in their system prompt, and only sees other agents' response text (not their system prompts or memories) in the conversation history.
...
- **No cross-agent queries**: Tool receives `current_agent` and can only create memories for that agent
- **Content limits**: 10,000 character max prevents abuse (enforced by model, not duplicated in tool)
...
| Decision | Rationale |
|----------|-----------|
| Single query for `memory_context` | Load memories once with `.to_a`, filter in Ruby to avoid N+1 |
| Hash key handling in `memories_count` | Use `.fetch("core", 0)` to handle enum key types correctly |
| Single unified tool | One tool with type parameter is cleaner than two near-identical tools |
| Model validation only | Tool rescues `ActiveRecord::RecordInvalid`, no duplicated validation |
| `AgentMemory.memory_types.key?` | More maintainable than hardcoded type checking |
