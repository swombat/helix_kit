...
class OpenRouterClaudeOpus4Test < ActiveSupport::TestCase

  test "gets response from anthropic/claude-opus-4 model" do
    VCR.use_cassette("openrouter_opus4_basic_response") do
      ...
      assert model_identifier.include?("claude") || model_identifier.include?("opus"),
            "Expected model identifier to include 'claude' or 'opus'"
  ...
  test "streams text response from anthropic/claude-opus-4 model" do
    VCR.use_cassette("openrouter_opus4_stream_text_response") do
  ...
  test "gets json response from anthropic/claude-opus-4 model" do
    VCR.use_cassette("openrouter_opus4_json_response") do
      response = OpenRouterApi.new.get_response(
        params: {
          model: "anthropic/claude-opus-4",
      ...
      assert response["model"].include?("claude") || response["model"].include?("opus"), "Expected model to include 'claude' or 'opus'"
  ...
  test "streams json response from anthropic/claude-opus-4 model" do
    VCR.use_cassette("openrouter_opus4_stream_json_response") do
      # Variables to track streaming
