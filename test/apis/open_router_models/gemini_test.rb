...
class OpenRouterGeminiTest < ActiveSupport::TestCase

  test "gets response from google/gemini-2.5-pro-preview-03-25 model" do
    VCR.use_cassette("openrouter_gemini_basic_response") do
      ...
      assert model_identifier.include?("gemini") || model_identifier.include?("Gemini"),
            "Expected model identifier to include 'gemini' or 'Gemini'"
  ...
  test "streams text response from google/gemini-2.5-pro-preview-03-25 model" do
    VCR.use_cassette("openrouter_gemini_stream_text_response") do
  ...
  test "gets json response from google/gemini-2.5-pro-preview-03-25 model" do
    VCR.use_cassette("openrouter_gemini_json_response") do
      response = OpenRouterApi.new.get_response(
        params: {
          model: "google/gemini-2.5-pro-preview-03-25",
      ...
      assert response["model"].include?("gemini") || response["model"].include?("Gemini"),
            "Expected model to include 'gemini' or 'Gemini'"

      # Verify we received content in the response
      text_content = response["choices"][0]["message"]["content"]
      assert text_content.present?

      # Gemini sometimes includes markdown formatting despite instructions
  ...
  test "streams json response from google/gemini-2.5-pro-preview-03-25 model" do
    VCR.use_cassette("openrouter_gemini_stream_json_response") do
      # Variables to track streaming
