module Chat::Contextualizable
  ...
  def context_messages_for(agent)
    ...
    recent_ids = context_messages.last(RECENT_TRANSCRIPT_MESSAGES).map(&:id)
    context_messages.select do |message|
      message.id > last_consolidated_message_id || recent_ids.include?(message.id)
    end
  end
  ...
  def checkpoint_context_message(timezone)
    ...
    {
      role: "user",
      content: "Summary of the conversation so far#{before}:\n\n#{checkpoint_summary}"
    }
  end
