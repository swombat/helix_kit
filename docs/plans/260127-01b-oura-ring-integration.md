...
**Date**: 2026-01-27
**Version**: 01b (Revised per DHH feedback)
**Status**: Draft
...
**Changes from 01a**: This revision applies DHH's feedback to simplify the implementation. Key changes include: simplified schema (7 columns removed), OAuth state stored in session instead of database, singular resource routing, simpler context injection via account owner, single enabled toggle instead of granular sharing, and extracted HTTP concern.
...
2. **Data Sync**: Background job fetches health data every 6 hours for enabled integrations
...
- [ ] **1.1 Create OuraIntegration migration**
  ...
  **Schema simplifications from 01a**:
  - Removed `oauth_state` and `oauth_state_expires_at` (now in session)
  - Removed `scopes` array (we always request same scopes - YAGNI)
  - Removed `share_sleep`, `share_readiness`, `share_activity` (single `enabled` toggle is sufficient)
  - Column naming: `access_token` not `access_token_ciphertext` (Rails encryption handles this)
...
- [ ] **2.1 Create OuraApi concern for HTTP operations**
  ...
  module OuraApi
    ...
    # Build authorization URL (state managed by controller/session)
    ...
    # Revoke token silently (best effort)
    def revoke_token
    ...
    def fetch_endpoint(path, start_date, end_date)
      ...
      if response.code == 401
        # Token was revoked or expired unexpectedly
...
- [ ] **3.1 Modify Chat#system_message_for to include health data**
  ...
  In `/app/models/chat.rb`, add health context injection. The simplest approach is via the account owner (no complex user lookup needed):
  ...
  This is simpler than looking up users from messages because:
  - Account owner is readily available via association
  - No N+1 query dance
  - Consistent health data across all messages in the conversation
...
- [ ] **5.1 Create OuraIntegrationController (singular)**
  ...
  class OuraIntegrationController < ApplicationController
    ...
    # Update settings (just enabled toggle)
    def update
      integration = Current.user.oura_integration
...
- [ ] **6.1 Add singular resource route**
  ...
  This gives us:
  - `GET /oura_integration` - show settings
  - `POST /oura_integration` - initiate OAuth
  - `PATCH /oura_integration` - update settings
  - `DELETE /oura_integration` - disconnect
  - `GET /oura_integration/callback` - OAuth callback
  - `POST /oura_integration/sync` - manual sync

### Phase 7: Frontend UI

- [ ] **7.1 Create simplified Oura integration settings page**
...
- [ ] **7.2 Add link from user settings navigation**
  ...
  Add to the user settings page or navigation:
...
- [ ] **8.2 Register OAuth application**

  At https://cloud.ouraring.com:
  - Set redirect URI: `https://yourdomain.com/oura_integration/callback`
...
- [ ] **10.1 Handle 401 responses**

  Already handled in `fetch_endpoint` - clears tokens and returns nil.

- [ ] **10.2 Rate limiting**

  Oura allows 5000 requests per 5 minutes. With 6-hourly syncs per user, this is unlikely to be hit. Add logging if it occurs:

  ```ruby
  # In OuraApi#fetch_endpoint
...
| File | Purpose |
|------|---------|
| `db/migrate/YYYYMMDDHHMMSS_create_oura_integrations.rb` | Database schema (simplified) |
...
| `app/controllers/oura_integration_controller.rb` | OAuth flow and settings (singular) |
| `app/jobs/sync_oura_data_job.rb` | Background data sync |
| `app/frontend/pages/settings/oura_integration.svelte` | Settings UI (simplified) |
...
2. **CSRF protection**: OAuth state stored in session with expiration
...
## Changes from Version 01a

| Aspect | 01a | 01b |
|--------|-----|-----|
| Schema columns | 12 columns | 7 columns |
| OAuth state | Stored in model | Stored in session |
| Sharing toggles | 3 granular (sleep/readiness/activity) | 1 (enabled) |
| Column naming | `access_token_ciphertext` | `access_token` |
| Route | `/oura_integrations` (plural) | `/oura_integration` (singular) |
| Context injection | Complex user lookup from messages | Simple `account.owner&.oura_health_context` |
| HTTP code | All in model | Extracted to OuraApi concern |
| Scopes column | Yes | Removed (YAGNI) |
| Error class | Outside model | Inside concern as `OuraApi::Error` |
