...
class MultiModelThinkingToolTest < ActiveSupport::TestCase
  ...
  test "multi-model conversation with thinking and tool use" do
    ...
    VCR.use_cassette("thinking/multi_model_tool_use", record: VCR_RECORD_MODE, match_requests_on: [ :method, :uri ]) do
  ...
  test "thinking agent can join conversation with non-thinking agent messages" do
    ...
    VCR.use_cassette("thinking/continuation_with_placeholder", record: VCR_RECORD_MODE, match_requests_on: [ :method, :uri ]) do
      # User message
      chat.messages.create!(
        role: "user",
        user: @user,
