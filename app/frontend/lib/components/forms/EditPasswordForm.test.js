...
describe('EditPasswordForm Component', () => {
  ...
  it('renders as a card component', () => {
    ...
    // Should be wrapped in a card (Form component provides this)
    const card = container.querySelector('.rounded-xl.border.shadow');
    expect(card).toBeInTheDocument();

    // Form framework handles max-width via container
    const formContainer = container.querySelector('.container.mx-auto');
  ...
  it('uses proper grid layout', () => {
    ...
    // Form framework uses space-y layout for fields
    const formContent = container.querySelector('.space-y-4');
