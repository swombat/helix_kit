...
class OpenRouterClaudeSonnet4Test < ActiveSupport::TestCase

  test "gets response from anthropic/claude-sonnet-4 model" do
    VCR.use_cassette("openrouter_sonnet4_basic_response") do
      ...
      assert model_identifier.include?("claude") || model_identifier.include?("sonnet"),
            "Expected model identifier to include 'claude' or 'sonnet'"
  ...
  test "streams text response from anthropic/claude-sonnet-4 model" do
    VCR.use_cassette("openrouter_sonnet4_stream_text_response") do
  ...
  test "gets json response from anthropic/claude-sonnet-4 model" do
    VCR.use_cassette("openrouter_sonnet4_json_response") do
      response = OpenRouterApi.new.get_response(
        params: {
          model: "anthropic/claude-sonnet-4",
      ...
      assert response["model"].include?("claude") || response["model"].include?("sonnet"), "Expected model to include 'claude' or 'sonnet'"
      ...
      assert json_data["creator"].include?("Yukihiro Matsumoto") || json_data["creator"].include?("Matz"),
            "Expected creator to include 'Yukihiro Matsumoto' or 'Matz'"
  ...
  test "streams json response from anthropic/claude-sonnet-4 model" do
    VCR.use_cassette("openrouter_sonnet4_stream_json_response") do
      # Variables to track streaming
