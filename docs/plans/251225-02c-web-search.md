...
5. Moving `require` statements to the top of `WebFetchTool` for consistency
...
### Step 4: Fix WebFetchTool require Statements

- [ ] Move `require` statements to top of `/app/tools/web_fetch_tool.rb`

The existing `WebFetchTool` has `require` statements inside the `execute` method. Move them to the top of the file for consistency with `WebSearchTool` and Ruby conventions.

```ruby
# Before (inside execute method):
def execute(url:)
  require "net/http"
  require "uri"
  # ...
...
| `app/tools/web_fetch_tool.rb` | Move `require` statements to top |
...
## Changes from Revision 2b

Based on DHH's final review feedback:

1. **Added Step 4 to fix WebFetchTool** - Move its `require` statements to the top of the file for consistency with Ruby conventions and the new `WebSearchTool`.

2. **Test configuration error through public interface** - Changed from testing private method with `send` to testing through `execute`:
   ```ruby
   # Before
...
3. **Cleaned up test helper** - Replaced the confusing `.gsub("example0", "rubyonrails")` hack with a clearer `sample_result` helper that explicitly handles the first result URL.
