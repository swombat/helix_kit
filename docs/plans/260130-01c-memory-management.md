...
Existing scopes (`for_prompt`, `active_journal`, `recent_first`) and existing methods (`expired?`) remain untouched.
...
- [ ] Test consolidation requires at least 2 IDs
- [ ] Test destroy atomicity in consolidate
- [ ] Test search sanitizes ILIKE wildcards
- [ ] Test delete operates on single ID only
...
7. **Single-ID consolidation**: Rejected with error requiring at least 2 IDs.
